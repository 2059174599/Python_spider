...
def get_encryption_obj(url, body, cookie_str=''):
    ts = khronos = str(time.time()).split(".")[0]
    _rticket = str(time.time() * 1000).split(".")[0]
    url = url.format(ts=ts, _rticket=_rticket)
    if body:
        ...
        stub = ""
    s = getXGon(url[url.index('?') + 1:], stub, cookie_str)
