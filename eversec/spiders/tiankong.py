...
class TiankongSpider(object):
    ...
    def __init__(self):
        self.name = 'huajin'
        self.start_urls = ['http://www.skycn.com/']
        ...
        self.page = 2
        self.html_url = 'http://www.skycn.com{}'
    ...
    def page_url(self):
        ...
        for url in self.parse():
            ...
            ehtml = etree.HTML(html)
            ids = ehtml.xpath('//div[@class="page-num"]/a[last()-1]/text()')
            # print('*****',ids)
            ids = ids[0]
            if ids:
                ids = int(ids)
    ...
    def list_url(self):
        ...
        for url in self.page_url():
            html = self.getRequest(url)
            ehtml = etree.HTML(html)
            for url in ehtml.xpath('//div[@class="list-pic"]/a/@href'):
    ...
    def shop_html(self):
        ...
        for url in self.list_url():
            ...
            name = ehtml.xpath('//h1/text()')
            item['name'] = name[0].strip()
            item['downLoadUrl'] = ehtml.xpath('//div[contains(@class,"soft-dl-button")]/a/@href')
            item['shop'] = '天空下载'
            item['url'] = url
