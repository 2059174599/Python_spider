...
class CompanySpider(scrapy.Spider):
 ...
 #    }
	...
	def parse(self, response):
		#item = LagouItem()
		tag_urls = response.xpath('//li[@class="wrapper"]//div/ul/li/a/@href').extract()
		next_page = response.xpath('//div[@class="pager_container"]/span[last()]/@class')
		for tag_url in tag_urls[:10]:
			tag_url = tag_url.replace('#order','')
			for i in range(1,10):
				page_url =  tag_url + '?first=false&pn=' + str(i)
				# print(tag_url)
				try:
					yield scrapy.Request(url=page_url,headers=self.headers,callback=self.next_page)
				except:
					pass
	...
	def next_page(self,response):
		...
		for each in response.xpath('//li[@class="company-item"]'):
			...
			yield scrapy.Request(url=lg_company_url,headers=self.headers,callback=self.company_page,meta={'item':item})


	def company_page(self,response):
		print('回调详情页:'+ response.url)
		#item = LagouItem()
		...
		company_word = response.xpath('//div[@class="company_word"]/text()').extract_first()
		item['company_url'] = company_url
		item['company_word'] = company_word.strip()
		item['img_log'] = 'http:' + img_log
		yield item